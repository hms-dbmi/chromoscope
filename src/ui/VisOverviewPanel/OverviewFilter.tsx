...
export const OverviewFilter = ({ title, options = [], onChange = null }: OverviewFilterProps) => {
    ...
    useEffect(() => {
        function handleClickOutside(event: MouseEvent) {
            const clickedOutside = !overviewFilterRef.current || !overviewFilterRef.current.contains(event.target as Node);
            if (clickedOutside) {
        ...
        switch (e.key) {
            ...
            case " ":
                ...
                if (focusedIndex >= 0 && focusedIndex < options.length) {
                    ...
                    setShowDropdown(false);
                    setFocusedIndex(-1);
                    if (onChange) onChange(selected.url.replace('https://chromoscope.bio/app/?showSamples=true&external=', ''));
                }
                break;
            case "Escape":
                e.preventDefault();
        ...
        >
            ...
            >
                <li 
                    ...
                    className={`dropdown-item ${selectedOption === "None" ? 'selected' : ''} ${focusedIndex === 0 ? 'focused' : ''}`}
                >
                    ...
                    .map((option, i) => {
                        return (
                            <li 
                                ...
                                className={`dropdown-item ${selectedOption === option.name ? 'selected' : ''} ${focusedIndex === i + 1 ? 'focused' : ''}`}
                            >
                                <span>
                                    {option.name}
                                </span>
